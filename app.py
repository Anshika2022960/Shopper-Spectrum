...
if menu == "🏠 Home":
    ...
    <div class="section-card">
        ...
        <p>Analyze sales, products, customers, and revenue patterns from the cleaned e-commerce dataset.</p>
    </div>
    """, unsafe_allow_html=True)

    total_transactions = eda_data["InvoiceNo"].nunique()
    total_customers = eda_data["CustomerID"].nunique()
    total_products = eda_data["Description"].nunique()
    total_revenue = eda_data["TotalAmount"].sum()
    ...
    country_sales = (
        eda_data.groupby("Country")["TotalAmount"]
        .sum()
    ...
    top_products = (
        eda_data.groupby("Description")["Quantity"]
        .sum()
        .sort_values(ascending=False)
        .head(10)
    )
    ...
    eda_data["Month"] = eda_data["InvoiceDate"].dt.to_period("M").astype(str)

    monthly_sales = (
        eda_data.groupby("Month")["TotalAmount"]
        .sum()
        .reset_index()
    )
    ...
    active_customers = (
        eda_data.groupby("CustomerID")["InvoiceNo"]
        .nunique()
        .sort_values(ascending=False)
        .head(10)
    )

    st.bar_chart(active_customers)
    ...
    st.bar_chart(customer_purchase.value_counts().head(20))

    st.subheader("💵 Transaction Amount Distribution")

    transaction_amount = eda_data["TotalAmount"]

    st.bar_chart(transaction_amount.value_counts().head(30))
